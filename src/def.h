...
// Sugestion: The macros above should be changed to something more like ...
// #define DEF_DEBUG
// #define DEF_WARNING
// #define DEF_ERROR
// #define DEF_INFO
...
// Sugestion: Update ANSI macros moving cursors to be more dynamic, i.e. ability to set how many moves
...
#else
#define ctz(u) ((u) & (1ul << 0) ? 0 : (u) & (1ul << 1) ? 1  \
                                   : (u) & (1ul << 2)   ? 2  \
                                   : (u) & (1ul << 3)   ? 3  \
                                   : (u) & (1ul << 4)   ? 4  \
                                   : (u) & (1ul << 5)   ? 5  \
...
#define DEBUGPRINTC(cond, _fmt, ...) \
  if (cond)                          \
  defprintf(DEBUGSTR _fmt, WHEREARG, ##__VA_ARGS__)
#define DEBUGDO(f) f
#else
...
#define ERRORPRINTC(cond, _fmt, ...) \
  if (cond)                          \
  defprintf(ERRORSTR _fmt, WHEREARG, ##__VA_ARGS__)
#define ERRORDO(f) f
#else
...
#define WARNINGPRINTC(cond, _fmt, ...) \
  if (cond)                            \
  defprintf(WARNSTR _fmt, WHEREARG, ##__VA_ARGS__)
...
#define INFOPRINTC(cond, _fmt, ...) \
  if (cond)                         \
  defprintf(INFOSTR _fmt, WHEREARG, ##__VA_ARGS__)
#define INFODO(f) f
#else
...
#ifdef __AVR__ 
    print_info("Architecture:", "AVR");
#endif

#ifdef __arm__ 
    print_info("Architecture:", "ARM 32-bit");
#endif

#ifdef __aarch64__ 
    print_info("Architecture:", "ARM 64-bit");
#endif
